    ...
    <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f9f9f9; padding: 40px; border-radius: 0 0 10px 10px;">
        ...
        <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
          <h3 style="margin: 0 0 20px 0;">${courseName}</h3>
...
export function paymentFailedEmailTemplate({ name, courseName, amount }: PaymentFailedEmailProps): string {
  return `
    ...
    <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f9f9f9; padding: 40px; border-radius: 0 0 10px 10px;">
        <p style="font-size: 18px;">Halo <strong>${name}</strong>,</p>
        <p>Mohon maaf, pembayaran Anda untuk kursus <strong>${courseName}</strong> sebesar <strong>Rp ${amount.toLocaleString('id-ID')}</strong> gagal diproses.</p>
...
export function passwordResetEmailTemplate({ name, resetUrl, expiresInMinutes }: PasswordResetEmailProps): string {
  return `
    ...
    <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f9f9f9; padding: 40px; border-radius: 0 0 10px 10px;">
        <p style="font-size: 18px;">Halo <strong>${name || 'Pengguna'}</strong>,</p>
